...
netflix_films  %>% 
  ...
  labs(fill = NULL, x = NULL, y = "Percentage of film",
       ...
       subtitle  = paste0(" Shannon Equitability Index: ", shannon_index_netflix)) + 
...
Another interesting thing that we could be interesting in is counting the number of words by category this because a word that is recurrent in a category will be very useful in the prediction.

As can be seen from the plot the most *documentary* seams to be present a lot in documentary, which is of course expected, I presume that Netflix descriptions starts with the word documentary. We can also notice that we have a problem with words like **find** and **finds**. This is a **stemming problem** but for the sake of this tutorial we will ignore this problem. If you are interested in what is stemming here the link to [the Wikipedia page](https://en.wikipedia.org/wiki/Stemming).
...
From now on we will only work on the training `films_train` and `films_test`will be used just to assess the quality of our model on new data.

A first type of analysis that we can make is performing a Principal Component Analysis. In order to do that we have to transform the word in a numerical matrix. A very effective way to do that is using **tf-idf**.
...
From the PCA we can obtain other information. Each principal component is a linear combination of the original columns. We can extract how each column (in our case word) influences each component looking at the loading.
...
Using the `recipes` a simple preprocessing recipe can be created. The a model in this case a linear support vector machine is created
...
In order to train the model on our dataset we can take advantage of the `tidymodel` ecosystem and create a `workflow object`
...
The accuracy achieved by this simple model is $68%$ which is not extraordinary but is anyway good enough considering the little effort we have put. What is very good is that this model achieves a very high *specificity* which means that we have very low false positive and for this reason we can trust what out models says.
...
Using the `vip` we can compute what are the features that influence the most our prediction. As we could expect *documentary* is the most important word but other word seems very interesting like *evil*, *christmas* *magic.*
...
films_imp %>% 
  ...
  guides(color = guide_colourbar(barwidth = 30, 
                                 title.position = "top")) +
